...
export default function ExtratoMovimentacoes({ usuarioCpf }) {
  ...
  async function fetchMovimentacoes() {
    try {
      // Envia o CPF do cliente clicado
      ...
      const movimentacoesFormatadas = dados.map((mov) => {
        const positivo = mov.receiverCpf === usuarioCpf
  ...
  <table style={{ width: '100%', borderCollapse: 'collapse', color: '#ffffffff' }}>
    <thead>
    ...
    <tbody>
      {movimentacoesPagina.map((mov, index) => (
        <tr key={index} style={{ borderBottom: '1px solid #eee' }}>
          <td style={{ padding: '8px', color: '#ffffffff' }}>{mov.data}</td>
          <td style={{ padding: '8px', color: '#ffffffff' }}>{mov.tipo}</td>
          <td
            style={{
              padding: '8px',
              textAlign: 'right',
